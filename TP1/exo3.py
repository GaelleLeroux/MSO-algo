...
def Valideur(fichier) :
    file = open(fichier, 'r')
    ouvrant = ListeChainee()
    ListeOuvrant = ['(','{','[','<']
    ListeFermant = [')','}',']','>']
    ok = True
    i = 1
    while ok:
        ...
        if not char:
            break
        if char in ListeOuvrant :
            ouvrant.inserer(char,0)
        if char in ListeFermant :
            if ouvrant.estVide() :
                ok = False
            else :
                ou = ouvrant.retourne()
                ouvrant.supprimer(0)
                ind = ListeOuvrant.index(ou)
                fer = ListeFermant[ind]
                if (char != fer) :
                    ok = False
